...
interface ProfileListProps {
    ...
    onSelectProfile: (id: string, checked: boolean) => void;
}
        ...
        <div>
            ...
            <table style={{
                ...
                <tr style={{background: '#f5f5f5'}}>
                    <th style={{padding: '8px 6px', textAlign: 'center'}}>Simulieren</th>
                ...
                {profiles.length === 0 ? (
                    <tr>
                        <td colSpan={8} style={{textAlign: 'center', padding: 12}}>No profiles found.</td>
                    </tr>
                ) : (
                    profiles.map(profile => (
                        <tr key={profile.id}
                            style={{borderBottom: '1px solid #eee', textAlign: 'center', verticalAlign: 'middle'}}>
                            ...
                            <td style={{
                                ...
                                textAlign: 'left'
                            }}>{profile.id.slice(0, 8)}</td>
                            <td style={{padding: '8px 6px', textAlign: 'right'}}>{profile.duration}</td>
                            <td style={{
                                padding: '8px 6px',
                                textAlign: 'right'
                            }}>{profile.startLat.toFixed(5)}, {profile.startLon.toFixed(5)}</td>
                            <td style={{
                                padding: '8px 6px',
                                textAlign: 'right'
                            }}>{profile.endLat.toFixed(5)}, {profile.endLon.toFixed(5)}</td>
                            <td style={{padding: '8px 6px', textAlign: 'center'}}>
                                <button
